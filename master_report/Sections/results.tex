...
This section contains the raw data of the results from simulations using Gazebo in Section \ref{sec:simulation} and real flight in the airport using the OptiTrack system as ground truth in Section \ref{sec:optitrack}. The results will be discussed and analyzed in Section \ref{sec:discussion}.     
...
The UAV where placed approximately 1.5 meters in front of the ArUco marker landing boards as seen in Figure \ref{fig:steady_state_aruco_pose_estimation_boards}. The reason for placing the UAV 1.5 meters and not 1.0 meter in front of the boards, which was done in the simulations, was because of blind angles in the OptiTrack system from where no ground truth was giving. Hence, to avoid this scenario, this pose were chosen. Each run lasted approximately 50 seconds where the pose estimations where compared to the ground truth for performance evaluation.
...
$3.15^{\circ}$ & $0.02^{\circ}$\\
 ...
 \label{tab:optitrack_steady_state_aruco_pose_estimation_boards}
\end{table}
...
The UAV was placed approximately 1.5 meters in front of the landing markers with the objective to keeps its pose for 30 seconds where the estimated ArUco marker pose where compared to that of the ground truth for performance evaluation.
...
An overall number of sixty vision based landings where performed. An illustration along with the procedure can be seen in Figure \ref{fig:optitrack_vision_based_landing_landing_stations}. This is not the exact same procedure as in the simulation in Section \ref{sec:vision_based_landing}. The changes have been made to make the landings using the three different landing boards as equal as possible because a reduced number of landings have been performed due to time constraints. 
...
\begin{table}[H]
    ...
    \addtolength{\leftskip} {-2cm}
    \addtolength{\rightskip}{-2cm}
        \caption{Statistics of the results from the vision based landing tests. Each test runs for 20 landings all with a WP error of 0.1 and vertical velocity of 0.2 $\frac{m}{s}$. As it may be seen, the UAV makes decent results in test two and three using landing boards two and three respectively. However, the landings deviates quite a lot in test one. This is clarified in Figure \ref{fig:optitrack_vision_based_landing_test}}
        \pgfplotstabletypeset[normal,
                columns/eg/.style={
                column name={Runs},
                dec sep align
