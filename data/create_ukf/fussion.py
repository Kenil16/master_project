...
class ukf():
    ...
    def __init__(self):
        ...
        self.covariance = [0.05, 0.05, 0.05, #Process noise x, y and z (pos)
                           1.04, 1.04, 0.04, #Process noise x, y and z (rate)
                           0.02, 0.02, 0.008, #Process noise x, y and z (acc)
                           1.5, 0.5, 0.05, #Process noise roll, pitch and yaw (angle)
                           0.9, 1.9, 0.5, #Process noise x, y and z (angle rate)
    ...
    def iterate_x(self, x, dt, inputs):
        ...
        ret[0] = x[0] + (self.prev_acc_x + (x[6]-self.prev_acc_x)/2)*dt
        ret[1] = x[1] + (self.prev_acc_y + (x[7]-self.prev_acc_y)/2)*dt
        ret[2] = x[2] + (self.prev_acc_z + (x[8]-self.prev_acc_z)/2)*dt
        ret[3] = x[3] + (self.prev_acc_x + (x[6]-self.prev_acc_x)/2)*dt
        ret[4] = x[4] + (self.prev_acc_y + (x[7]-self.prev_acc_y)/2)*dt
        ret[5] = x[5] + (self.prev_acc_z + (x[8]-self.prev_acc_z)/2)*dt
        ret[6] = x[6]
        ret[7] = x[7]
        ret[8] = x[8]
    ...
    def main(self):
        ...
        for j in range(m):
            ...
            row = [i[3,j], i[4,j], i[5,j], i[9,j], i[10,j], i[11,j]]
            ...
            if corrected_acc_z < 0.15 and corrected_acc_z > -0.15:
            ...
            self.gyro_x = self.gyro_x + row[3]*dt
            self.gyro_y = self.gyro_y + row[4]*dt
            self.gyro_z = self.gyro_z + row[5]*dt
