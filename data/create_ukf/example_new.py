...
class ukf():
    ...
    def __init__(self):
        ...
        self.covariance = [0.05, 0.05, 0.05, #Process noise x, y and z (pos)
                           0.004, 0.004, 0.004, #Process noise x, y and z (rate)
    ...
    def iterate_x(self, x, dt, inputs):
        ...
        """
        if x[6] < 0.01 and  x[6] > -0.01:
            x[3] = 0.000006

        if x[7] < 0.01 and  x[7] > -0.01:
            x[4] = 0.000006
        
        if x[8] < 0.01 and  x[8] > -0.01:
            x[5] = 0.000006
        """

        ret[0] = x[0] + x[3]*dt #(self.prev_vel_x + (x[3]-self.prev_vel_x)/2)*dt**1
        ret[1] = x[1] + x[4]*dt #(self.prev_vel_y + (x[4]-self.prev_vel_y)/2)*dt**1
        ret[2] = x[2] + x[5]*dt #(self.prev_vel_z + (x[5]-self.prev_vel_z)/2)*dt**1
        ret[3] = x[6]*1.0
        ret[4] = x[7]*1.0
        ret[5] = x[8]*1.0
        ret[6] = x[6]
    ...
    def main(self):
        ...
        for j in range(m):
            ...
            imu_accX = corrected_acc_x #- self.prev_acc_x_new #row[3]
            imu_accY = corrected_acc_y #- self.prev_acc_y_new#row[4]
            imu_accZ = corrected_acc_z #- self.prev_acc_z_new#row[5]
            ...
            self.prev_acc_z_new = corrected_acc_z
