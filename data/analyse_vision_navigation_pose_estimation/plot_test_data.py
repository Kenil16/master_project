...
class plot_data():
    ...
    def read_dir_files(self, path):
        ...
        for fle in files:
            ...
            #Ground truth data
            g_x = np.array([item[21] for item in self.data])
            g_y = np.array([item[22] for item in self.data])
            g_z = np.array([item[23] for item in self.data])
            g_roll = np.array([item[18] for item in self.data])
            g_pitch = np.array([item[19] for item in self.data])
            g_yaw = np.array([item[20] for item in self.data])
